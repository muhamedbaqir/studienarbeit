...
static void brd_submit_bio(struct bio *bio)
{
	...
	// hang at sector hang_sector
	sector_t start = bio->bi_iter.bi_sector;
	sector_t end   = start + (bio_sectors(bio) - 1);

	if (start <= hang_sector && end >= hang_sector) {
		pr_warn("Simulating slow bio at sector %llu\n", (unsigned long long)hang_sector);

		// safe delay
		msleep(60000);          // sleep an hour
		bio_io_error(bio);     // complete with -EIO
		return;
	}
	bio_endio(bio);
}
