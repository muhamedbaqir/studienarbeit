...
static sector_t fault_sector = 100;
...
static void brd_submit_bio(struct bio *bio)
{
	...
	// forcing a -EBIO when hitting sector: fault_sector
	sector_t start = bio->bi_iter.bi_sector;
    sector_t end   = start + (bio_sectors(bio) - 1);

    if (start <= fault_sector && end >= fault_sector) {
		printk("BRD: Returning EBIO, hit sector 100");
        bio_io_error(bio);                    /* completes with -EIO */
        return;
    }
